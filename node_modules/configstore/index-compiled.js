...
Configstore.prototype = _Object$create(Object.prototype, {
	all: {
		get: function get() {
			try {
		...
		set: function set(val) {
			try {
				...
				writeFileAtomic.sync(this.path, _JSON$stringify(val, null, '\t'), writeFileOptions);
			} catch (err) {
				// improve the message of permission errors
				if (err.code === 'EACCES') {
					err.message = err.message + '\n' + permissionError + '\n';
	...
	size: {
		get: function get() {
			return _Object$keys(this.all || {}).length;
		}
	}
});
...
Configstore.prototype.set = function (key, val) {
	...
	if (arguments.length === 1) {
		_Object$keys(key).forEach(function (k) {
