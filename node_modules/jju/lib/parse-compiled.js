...
function parse(input, options) {
  ...
  */
  ...
  function fail(msg) {
    ...
    if (!msg) {
      if (position < length) {
        var token = '\'' + _JSON$stringify(input[position]).replace(/^"|"$/g, '').replace(/'/g, "\\'").replace(/\\"/g, '"') + '\'';
  ...
  function parseObject() {
    var result = options.null_prototype ? _Object$create(null) : {},
        empty_object = {},
        is_non_empty = false;
    ...
    while (position < length) {
      ...
      if (chr === '}' && item1 === undefined) {
        ...
        if ((item1 in empty_object || empty_object[item1] != null) && options.reserved_keys !== 'replace') {
          ...
          if (item2 !== undefined) {
            is_non_empty = true;
            _Object$defineProperty(result, item1, {
              value: item2,
              enumerable: true,
              configurable: true,
              writable: true
