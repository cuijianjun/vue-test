...
function morgan(format, options) {
  ...
  if (format && (typeof format === 'undefined' ? 'undefined' : _typeof(format)) === 'object') {
    ...
    deprecate('morgan(options): use morgan(' + (typeof fmt === 'string' ? _JSON$stringify(fmt) : 'format') + ', options) instead');
  }
...
function compile(format) {
  ...
  var js = '  "use strict"\n  return "' + fmt.replace(/:([-\w]{2,})(?:\[([^\]]+)\])?/g, function (_, name, arg) {
    ...
    var tokenFunction = 'tokens[' + String(_JSON$stringify(name)) + ']';

    if (arg !== undefined) {
      tokenArguments += ', ' + String(_JSON$stringify(arg));
    }
